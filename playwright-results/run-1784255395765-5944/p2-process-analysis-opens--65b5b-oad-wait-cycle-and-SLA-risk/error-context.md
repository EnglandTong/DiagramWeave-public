...
+     1 pages · 0 unreachable · 0 bottlenecks · 0 cycles · 0 SLA risks
+     Page 1Critical path / 关键路径None / 无Unreachable / 不可达0Bottlenecks / 瓶颈0Longest wait / 最长等待Not configured / 未配置Cycles / 循环0SLANot configured / 未配置Role load / 角色负荷None / 无Targets / 目标: 
+   
...
- dialog "Process Analysis / 流程分析":
  ...
  - text: Process Analysis / 流程分析 1 pages · 0 unreachable · 0 bottlenecks · 0 cycles · 0 SLA risks
  - heading "Page 1" [level=3]
...
```ts
  ...
  6  |   await page.evaluate(() => loadFlowDocumentPayload({ schemaVersion: 3, slaDays: 4, currentPageId: 'p1', pages: [{ id: 'p1', name: 'Operations', slaDays: 4, nodes: [
