...
Call log:
  - waiting for locator('.mobile-mode-bar button').filter({ hasText: /演示/ })
    - locator resolved to <button type="button" aria-label="开始演示" onclick="enterPresentation()">演示</button>
  - attempting click action
    2 × waiting for element to be visible, enabled and stable
      - element is visible, enabled and stable
      - scrolling into view if needed
      - done scrolling
      - <div id="reviewOverlay" aria-hidden="false" class="confirm-overlay visible" onclick="if(event.target===this)hideReviewPanel()">…</div> intercepts pointer events
