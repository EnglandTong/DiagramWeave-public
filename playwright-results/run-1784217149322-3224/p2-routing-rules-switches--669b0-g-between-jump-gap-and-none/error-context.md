...
+ Array [
+   "M 180 110 L 480 350",
+   "M480,110 L34,110 L34,350 L180,350",
+ ]
```
...
```ts
  ...
  52 |   const debugPaths = await page.locator('.connection-line').evaluateAll(lines => lines.map(line => line.getAttribute('d')));
> 53 |   expect(debugPaths, 'crossing fixture paths').toEqual([]);
     |                                                ^ Error: crossing fixture paths
  54 |   await expect(page.locator('.connection-bridge')).not.toHaveCount(0);
  55 |   await page.locator('#routingBridgeBehavior').selectOption('gap');
  56 |   await expect(page.locator('.connection-bridge')).toHaveCount(0); await expect(page.locator('.connection-bridge-gap')).not.toHaveCount(0);
  57 |   await page.locator('#routingBridgeBehavior').selectOption('none');
  58 |   await expect(page.locator('.connection-bridge-gap')).toHaveCount(0);
  59 | });
  60 | 
```