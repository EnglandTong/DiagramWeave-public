...
- generic [ref=e1]:
  ...
  - dialog "Stencil Manager / 图形包管理" [ref=e378]:
    ...
    - alert [ref=e382]
    - generic [ref=e384]:
      - checkbox "Enable Renamed Operations" [checked] [ref=e385]
      - generic [ref=e386]:
        - textbox "Pack name" [ref=e387]: Renamed Operations
  ...
  - dialog "开始项目" [ref=e394]:
    - button "关闭" [active] [ref=e395] [cursor=pointer]:
      - img [ref=e396]
    - generic [ref=e399]: 开始项目
    - generic [ref=e400]: 请选择保存新档案、打开旧档案，或暂不处理。
    - generic [ref=e401]:
      - button "取消" [ref=e402] [cursor=pointer]: 暂不处理
...
> 28 |   const download = page.waitForEvent('download');
     |                         ^ Error: page.waitForEvent: Test timeout of 120000ms exceeded.
  29 |   await page.getByRole('button', { name: 'Export' }).click();
  30 |   expect((await download).suggestedFilename()).toBe('qa_ops.stencil.json');
  31 |   await expect.poll(() => page.evaluate(() => DiagramWeave.stencilPacks.list()[0].name)).toBe('Renamed Operations');
