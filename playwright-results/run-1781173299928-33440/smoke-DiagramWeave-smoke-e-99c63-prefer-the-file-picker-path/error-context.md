...
Call log:
  - waiting for locator('button[onclick="showExcelDataDialog()"]')
    - locator resolved to <button onclick="showExcelDataDialog()" data-i18n-title="toolbar.excel" title="Excel template & import" class="toolbar-btn icon-only toolbar-hide-in-present">…</button>
...
```ts
  ...
  31 |       Object.defineProperty(window, 'showOpenFilePicker', {
  32 |         configurable: true,
  33 |         writable: true,
  34 |         value: async () => {
  35 |           window.__dwPickerCalls += 1;
  36 |           throw { name: 'AbortError' };
  37 |         },
  38 |       });
  39 |       HTMLInputElement.prototype.showPicker = function () {
  40 |         throw new Error('showPicker fallback should not be used in this test');
  41 |       };
  42 |       HTMLInputElement.prototype.click = function () {
  43 |         throw new Error('click fallback should not be used in this test');
  44 |       };
  45 |     });
  46 | 
  47 |     await page.goto('/flowchart-editor.html');
> 48 |     await page.locator('button[onclick="showExcelDataDialog()"]').click();
     |                                                                   ^ Error: locator.click: Test timeout of 15000ms exceeded.
  49 |     await page.locator('button[onclick="triggerExcelUpload()"]').click();
  50 |     await page.locator('button[onclick="triggerProjectExcelUpload()"]').click();
  51 | 
  52 |     await expect.poll(async () => page.evaluate(() => window.__dwPickerCalls)).toBe(2);
  53 |   });
  54 | });
  55 | 
```